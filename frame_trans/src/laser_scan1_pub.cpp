...
int main(int argc, char * argv[]) {
    ros::init(argc, argv, "ydlidar1_node"); 
    ...
    ros::NodeHandle nh;
    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>("laser_scan1", 1000);
    ros::NodeHandle nh_private("~");
    nh_private.param<std::string>("port", port, "/dev/ttyUSB0"); 
    nh_private.param<int>("baudrate", baudrate, 115200); 
    nh_private.param<std::string>("frame_id", frame_id, "laser_frame1");
    ...
    nh_private.param<double>("frequency", _frequency , 12.0);
    ...
    laser.turnOff();
    printf("[YDLIDAR INFO] Now YDLIDAR1 is stopping .......\n");
    laser.disconnecting();
    return 0;
}
