...
int main(int argc, char * argv[]) {
    ros::init(argc, argv, "ydlidar2_node"); 
    ...
    ros::NodeHandle nh;
    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>("laser_scan2", 1000);
    ros::NodeHandle nh_private("~");
    nh_private.param<std::string>("port", port, "/dev/ttyUSB1"); 
    nh_private.param<int>("baudrate", baudrate, 115200); 
    nh_private.param<std::string>("frame_id", frame_id, "laser_frame2");
    ...
    laser.turnOff();
    printf("[YDLIDAR INFO] Now YDLIDAR2 is stopping .......\n");
    laser.disconnecting();
    return 0;
}
